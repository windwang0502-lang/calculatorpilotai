...
export default function FreightClassCalculator() {
  ...
  return (
    <ToolLayout
      ...
      description="Calculate your shipment's density and get an estimated freight class for LTL shipping quotes."
    >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex bg-slate-100 rounded-lg overflow-hidden text-xs font-bold">
              {(['in', 'cm'] as const).map(u => (
                <button key={u} onClick={() => setUnit(u)} className={`px-3 py-1.5 uppercase tracking-wider transition-colors ${unit === u ? 'bg-primary text-primary-foreground' : 'text-slate-500 hover:text-slate-800'}`}>
                  {u === 'in' ? 'in / lb' : 'cm / kg'}
                </button>
              ))}
            </div>
