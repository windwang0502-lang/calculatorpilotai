...
export default function UPSDimWeightCalculator() {
  ...
  const validate = () => {
    ...
    if (l <= 0 || l > 165) newErrors.length = 'Length must be 1-165 in (UPS limit)';
    if (w <= 0 || w > 165) newErrors.width = 'Width must be 1-165 in (UPS limit)';
    if (h <= 0 || h > 108) newErrors.height = 'Height must be 1-108 in (UPS limit)';
    setErrors(newErrors);
  ...
  const faqs = [
    { question: 'What is the UPS DIM divisor?', answer: 'UPS uses a DIM divisor of 139 for daily rates and 166 for retail rates. The daily rate divisor typically results in lower shipping costs.' },
    { question: 'How does UPS calculate billable weight?', answer: 'UPS compares actual weight to dimensional weight. For packages over 5,184 cubic inches, they may use the greater of DIM weight or billable weight based on package dimensions.' },
    { question: 'What are UPS package size limits?', answer: 'UPS packages can be up to 165 inches in length and 150 lbs. Packages over 130 inches in combined length and girth incur oversize surcharges.' },
    { question: 'Does UPS charge for oversize packages?', answer: 'Yes. UPS applies surcharges for packages over 130 inches in combined length and girth, and additional fees for packages exceeding 96 inches in length.' },
  ];
    ...
    '@graph': [
      {
        ...
        name: 'UPS DIM Weight Calculator',
        description: 'Calculate dimensional weight and billable shipping costs for UPS packages. Determine UPS shipping charges based on package dimensions.',
        url: 'https://www.calculatorpilotai.com/tools/shipping/ups-dim-weight-calculator',
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'UPS DIM Weight Calculator', item: 'https://www.calculatorpilotai.com/tools/shipping/ups-dim-weight-calculator' },
        ],
      },
    ],
  };
  ...
  return (
    <ToolLayout toolId="ups-dim" category="shipping" canonical="https://www.calculatorpilotai.com/tools/shipping/ups-dim-weight-calculator">
      ...
      <section className="space-y-8">
        <div className="bg-white p-8 border border-slate-200 rounded-xl shadow-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-amber-600 text-black px-3 py-1 rounded font-bold text-sm">UPS</div>
                <h2 className="text-2xl font-bold">DIM Weight Calculator</h2>
              </div>
              <p className="text-slate-600 text-sm">Calculate dimensional weight for UPS shipments</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none font-mono text-lg ${errors.length ? 'border-rose-500' : 'border-slate-200'}`}
              />
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none font-mono text-lg ${errors.width ? 'border-rose-500' : 'border-slate-200'}`}
              />
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none font-mono text-lg ${errors.height ? 'border-rose-500' : 'border-slate-200'}`}
              />
          ...
          <div className="mt-6 p-4 bg-slate-50 rounded-lg">
            ...
            <select
              ...
              className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none font-mono"
            >
              <option value={139}>139 - UPS Daily Rate</option>
              <option value={166}>166 - UPS Retail Rate</option>
              <option value={194}>194 - UPS Ground</option>
            </select>
            <p className="text-xs text-slate-500 mt-2">UPS uses 139 for daily rates, 166 for retail/invoice rates.</p>
          </div>

          <button
            onClick={handleCalculate}
            className="mt-6 w-full bg-amber-600 text-black font-bold py-4 rounded-lg hover:bg-amber-700 transition-colors uppercase tracking-widest"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="p-6 bg-amber-100 border border-amber-200 rounded-xl text-center">
            <span className="text-xs font-bold text-amber-700 uppercase tracking-widest block mb-2">Billable Weight</span>
            <div className="text-2xl font-mono font-bold text-amber-700 tabular-nums">{formatNumber(billableWeight, { decimals: 0 })} lbs</div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-3xl mx-auto space-y-6">
          <h2 className="text-3xl font-bold text-center">How to Use the UPS DIM Calculator</h2>
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-6">
            <h3 className="font-bold mb-3 text-lg">UPS Package Limits</h3>
